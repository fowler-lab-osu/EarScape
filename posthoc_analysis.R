...
postHoc <- function(cross = crossType){

    glm_data <- read.csv(paste("Posthoc_Analysis/for_posthoc_analysis_",cross,".csv", sep=""))
    ...
    write.csv(sigQuad,paste("Posthoc_Analysis/quad_ears_", cross, ".csv",sep=""), row.names = FALSE)
    ...
    write.csv(out_df,paste("Posthoc_Analysis/posthoc_categories_",cross,".csv",sep=""), row.names = FALSE)
    ...
    write.csv(trend_counts, paste("Posthoc_Analysis/pattern_counts_", cross,".csv",sep=""), row.names = FALSE)
}
