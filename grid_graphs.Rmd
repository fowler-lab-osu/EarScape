---
title: "Grid Spatial Graphs for Paper"
...
```{r}
list_alleles <- list.dirs("/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/SpatialAnalysis_Alleles_AmbigAdjust", full.names = FALSE, recursive = FALSE)
```
...
for (i in 1:length(list_alleles)){
  ...
  filenames[[i]][[2]] = list.files(paste0("/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/SpatialAnalysis_Alleles_AmbigAdjust/", list_alleles[[i]]), pattern="*.xml", full.names=TRUE) #Getting path to observation.
...
for (i in 1:length(bin_ed)) {
  p1 <- ggplot(data = bin_w_trate_ed[[i]], aes(x=bins, y = rate)) +
...
nN <- length(list_plots)
nCol <- floor(sqrt(nN))

#png(file = paste("/Users/michellebang/st research lab/spatialanalysis/SpatialAnalysisMichelle/spatial_graphs_files/", filenames[[n]][[1]], "_grid.png", sep = ""))

grid.arrange(grobs = list_plots, ncol = nCol, top = filenames[[n]][[1]])
