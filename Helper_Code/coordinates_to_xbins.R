...
coordinates_to_xbins <- function(input_df, num_bins) {
  ...
  for (val in sequence) {                                              #for each value                                      
    ...
    if ((input_df$x[val] == final_df$bins[num_bins+1]) ) {  #dealing with edge-case of kernel with defines the boundary of final bin
       if (input_df$type[val] == "WT") {                            
          final_df$WT[num_bins] = final_df$WT[num_bins] + 1                   
        }               
        else if (input_df$type[val] == "GFP") {
          final_df$GFP[num_bins] = final_df$GFP[num_bins] + 1
        } 
    }
  }
