...
dir.create(paste("./Allele_Spatial_Plots/",cross, sep=""))
...
for (n in 1:length(filenames)){
  ...
  pdf(file = paste("./Allele_Spatial_Plots/",cross,"/", filenames[[n]][[1]], "_coord_plots.pdf", sep = ""),
    width = 5.8,
    height = 3,)
...
for (n in 1:length(filenames)){
  ...
  bin_data = lapply(coords, function(x) {coordinates_to_xbins(x,binNumber)} )
  ...
  for (i in sequence) {
      bin_ed[[i]] = data.frame("bins" = bin_data[[i]]$bins[2:(binNumber-1)], "WT" = bin_data[[i]]$WT[2:(binNumber-1)] , "GFP" = bin_data[[i]]$GFP[2:(binNumber-1)])
  }
  
  titleList <- filenames[[n]][[3]]
  
  pdf(file = paste("./Allele_Spatial_Plots/",cross,"/", filenames[[n]][[1]], "_bin_graphs.pdf", sep = ""),
    width = 5.8,
    height = 3,)
...
degreesOfFreedom = binNumber - 2 - 2    #df = 12 when bins=16
lower_tail_pvals = lapply(t_stats, function(x) {pt(x, df=degreesOfFreedom, lower.tail = TRUE)}) #probs P[<t]  
upper_tail_pvals = lapply(t_stats, function(x) {pt(x, df=degreesOfFreedom, lower.tail = FALSE)}) #probs P[>=t]  
...
pdf(file = paste("./Allele_Spatial_Plots/",cross,"/", filenames[[n]][[1]], "_xcoordsbytr_linear_ends.pdf", sep = ""),
    width = 5.8,
    height = 3,)
...
ggsave(paste("./Allele_Spatial_Plots/",cross,"/",filenames[[n]][[1]],"_trends_lin_glm.png",sep=""), units=unit, width=w, height=h, dpi=dpi)
ggsave(paste("./Allele_Spatial_Plots/",cross,"/",filenames[[n]][[1]],"_trends_lin_glm.pdf",sep=""), units=unit, width=w, height=h)

#Quadratic Plots
pdf(file = paste("./Allele_Spatial_Plots/",cross,"/", filenames[[n]][[1]], "_xcoordsbytr_quad_ends.pdf", sep = ""),
    width = 5.8,
    height = 3,)
